...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mcrmods="xalan://org.mycore.mods.classification.MCRMODSClassificationSupport"
  ...
  <xsl:template match="user" mode="output">
    ...
    <xsl:value-of select="concat('Forschungsgruppe: ',$groupmember/label[@xml:lang='de']/@text,$newline)" />
  ...
  <xsl:template match="mycoreobject" mode="mailReceiver">
    <!-- send mail to EditorMail only if WIAS specific group contact not exists -->
    <xsl:if test="string-length($MCR.mir-module.EditorMail) &gt; 0 and string-length($groupcontact) = 0">
      <xsl:for-each select="str:tokenize($MCR.mir-module.EditorMail,',')" >
        <to> <xsl:value-of select="." /> </to>
      </xsl:for-each>
    </xsl:if>
    <!-- send mail to WIAS specific group contact if exists -->
    <xsl:if test="string-length($groupcontact) &gt; 0">
      <xsl:for-each select="str:tokenize($groupcontact,',')" >
        <to> <xsl:value-of select="." /> </to>
      </xsl:for-each>
    </xsl:if>
